...
export default async function CourseDetailsPage({
  ...
  const totalDuration = course.modules.reduce(
    (acc, mod) => acc + mod.lessons.reduce((a, l) => a + l.durationInMs, 0),
    0
  )
    ...
    <section className="flex flex-col">
      <div className="flex justify-between gap-6 flex-col md:flex-row">
        ...
        <Image
          ...
          width={300}
          height={400}
          className="aspect-video rounded-2xl border border-primary object-cover"
        />
      </div>
        ...
        <Tabs defaultValue="overview">
          ...
          <TabsContent value="content" className="mt-4 flex flex-col gap-6">
            {course.modules.map((mod, index) => (
              <div key={mod.id} className="flex gap-4 bg-muted p-4 rounded-2xl">
