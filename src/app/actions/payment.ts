...
export const createPixCheckout = async (payload: PixCheckoutSchema) => {
  ...
  try {
    ...
    // Log útil para debug (não vaza segredo)
    console.error("Asaas error (PIX):", error?.response?.status, error?.response?.data);
    ...
    if (status === 401 || status === 403) {
      throw new ServerError({
        code: "NOT_AUTHORIZED",
        message:
          "Integração de pagamento não autorizada. Verifique a chave da API do Asaas no .env do container.",
      });
    }
    ...
    if (firstErrorDescription) {
      throw new ServerError({
        code: "FAILED_TO_CREATE_PAYMENT",
        message: `Falha ao processar o pagamento: ${firstErrorDescription}`,
      });
    }
...
export const createCreditCardCheckout = async (
  ...
  try {
    ...
    console.error("Asaas error (CC):", error?.response?.status, error?.response?.data);
...
export const getPixQrCode = async (invoiceId: string) => {
  ...
  try {
    ...
    if (!isAxiosError(error)) {
      throw new ServerError({
        message: "Falha ao buscar QR Code do Pix",
        code: "FAILED_TO_GET_QRCODE",
      });
    }

    console.error(
      "Asaas error (PIX QRCODE):",
...
export const getInvoiceStatus = async (invoiceId: string) => {
  ...
  try {
    ...
    if (!isAxiosError(error)) {
      throw new ServerError({
        message: "Falha ao consultar status do pagamento",
        code: "FAILED_TO_GET_INVOICE",
      });
    }
